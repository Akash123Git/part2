...
import Employee from './Rowcomponent'
...
class App extends React.Component {
  ...
  constructor(){
    super();
    this.state={
      title:"importing java codes",
      increments:5,
      city:["bangalore","mysore","udupi"]
    }
}
...
check=()=>{
  ...
  this.setState({title:"lodaing......"})
  if(this.state.increments===0){
    clearInterval(interval);
    this.setState({title: "java codes are imported"})
  }
  },1000)
}
  ...
  this.setState({city:[...this.state.city,city]})
    ...
    render(){
        return <div>
          ...
          <h1>welcome to java word</h1>
          <hr/>
          <p>{this.state.title}</p>
        <button onClick={this.check}>load java codes</button>
